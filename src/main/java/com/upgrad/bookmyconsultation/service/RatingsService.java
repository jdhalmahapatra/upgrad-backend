...
public class RatingsService {
	...
	public void submitRatings(Rating rating) {
		...
		String docId = rating.getDoctorId();
		Optional<Doctor> doc = doctorRepository.findById(docId);
		double existingRating  = doc.get().getRating();
		
		Double newRating = (existingRating + rating.getRating())/2;
		doc.get().setRating(newRating);
		doctorRepository.save(doc.get());
	}
